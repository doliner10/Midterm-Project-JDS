...
def nyc_calculate_average(cut_data):
    ...
    for cut_dict in cut_data:
        rate = "Rate"
        if cut_dict["Average Rate"] == "$1.25 1st Hour / $2.00 2nd Hour":
            cut_dict[rate] = float(1.625)
        if cut_dict["Average Rate"] == "$1.25 1st Hour / $2.00 2nd Hour (Monday - Thursday) / $1.25 Per Hour (Friday)":
            cut_dict[rate] = float(1.4375)
        if cut_dict["Average Rate"] == "$1.25 Per Hour":
            cut_dict[rate] = float(1.25)
        if cut_dict["Average Rate"] == "$1.50 1st Hour / $2.50 2nd Hour":
            cut_dict[rate] = float(2.00)
        if cut_dict["Average Rate"] == "$1.50 1st Hour / $2.50 2nd Hour (Monday - Thursday) / $1.50 Per Hour (Friday)":
            cut_dict[rate] = float(1.75)
        if cut_dict["Average Rate"] == "$1.50 1st Hour / $2.50 2nd Hour / $3.00 3rd Hour":
            cut_dict[rate] = float(2.33)
        if cut_dict["Average Rate"] == "$1.50 Per Hour":
            cut_dict[rate] = float(1.50)
        if cut_dict["Average Rate"] == "$2.00 1st Hour / $4.00 2nd Hour":
            cut_dict[rate] = float(3.00)
        if cut_dict["Average Rate"] == "$2.00 Per Hour":
            cut_dict[rate] = float(2.00)
        if cut_dict["Average Rate"] == "$2.50 1st Hour / $4.00 2nd Hour":
            cut_dict[rate] = float(3.25)
        if cut_dict["Average Rate"] == "$2.50 Per Hour":
            cut_dict[rate] = float(3.25)
        if cut_dict["Average Rate"] == "$3.00 1st Hour / $3.00 2nd Hour / $2.00 Per Additional Hour":
            cut_dict[rate] = float(2.33)
        if cut_dict["Average Rate"] == "$4.00 1st Hour / $6.75 2nd Hour":
            cut_dict[rate] = float(5.375)
        if cut_dict["Average Rate"] == "$4.00 Per Hour":
            cut_dict[rate] = float(4.00)
        if cut_dict["Average Rate"] == "$4.50 1st Hour / $7.50 2nd Hour":
            cut_dict[rate] = float(6.00)
        if cut_dict["Average Rate"] == "$4.50 1st Hour / $7.50 2nd Hour (Before 6pm) / $4.50 Per Hour (After 6pm)":
            cut_dict[rate] = float(5.25)
        if cut_dict["Average Rate"] == "$4.50 Per Hour":
            cut_dict[rate] = float(4.50)
        if cut_dict["Average Rate"] == "N/A":
            cut_dict[rate] = float(0)
    for cut_dict in cut_data:
        del cut_dict["Average Rate"]
